...
def schedule_url_for_dug(dug_num: int, year: int = None, base_url: str = "https://cfp.3mdeb.com") -> str:
    if year is None:
...
def normalize_time(start_raw: str, date_full: str = None, offset_minutes: int = 60) -> str:
    dt = None
    if date_full:
        try:
...
def extract_events(root: ET.Element, dug_num: int = None, slot_times: list | None = None, time_offset_minutes: int = 60):
            ...
            if name and name.strip() == 'Dasharo User Group':
                ...
                for idx, ev in enumerate(room_events):
                    ...
                    if idx < len(slot_times):
                        ...
                        start = normalize_time(start_raw, date_full, time_offset_minutes)
...
def render_template(template_path: str, dug_num: int, events, banner_width: str = '650px') -> str:
...
def main(argv=None):
    ...
    mutex.add_argument('--schedule', help='Path or URL to schedule.xml (overrides --dug-num)')
    mutex.add_argument('--dug-num', type=int, help='DUG number (e.g. 10)')
    p.add_argument('--year', type=int, help='Year used to build the vPub slug (default: current year)')
    p.add_argument('--template', default='templates/1-greetings-agenda.j2', help='Jinja2 template path')
    p.add_argument('--output', default=None, help='Output file or directory (default: stdout)')
    p.add_argument('--banner-width', default='650px', help='Banner width, e.g. 650px')
    p.add_argument('--force', action='store_true', help='Allow overwriting pages/dug_<n>/1-greetings-agenda.md')
    p.add_argument('--time-offset-minutes', type=int, default=60, help='Apply offset to times (minutes)')
    p.add_argument('--base-url', default='https://cfp.3mdeb.com', help='Base URL for pretalx instance')
    ...
    events = extract_events(root, dug_num=args.dug_num, time_offset_minutes=args.time_offset_minutes)
    rendered = render_template(args.template, args.dug_num or 0, events, banner_width=args.banner_width)

    outpath = ensure_output_path(args.output, args.dug_num or 0)
    if outpath:
        repo_expected = None
        if args.dug_num:
            repo_expected = os.path.abspath(os.path.join('pages', f'dug_{args.dug_num}', '1-greetings-agenda.md'))
        if repo_expected and os.path.abspath(outpath) == repo_expected and os.path.exists(outpath) and not args.force:
            print(f"Refusing to overwrite repository page {repo_expected}. Pass --force to overwrite.", file=sys.stderr)
            sys.exit(2)
