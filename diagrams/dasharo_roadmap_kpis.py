...
bars_dspr = plt.bar(dates, dspr_values, color='#272727', label='Dasharo Support Package Releases')
bars_dcr = plt.bar(dates, dcr_values, bottom=dspr_values, color='#38d430', label='Dasharo Community Releases')

plt.title('Number of Dasharo Public Releases in time', fontsize=18, fontweight='bold', color='#272727')
...
for i in range(len(dates)):
    if dcr_values[i] != 0 and dspr_values[i] != 0:
        ...
        plt.annotate(f'{total}',
                     xy=(bars_dcr[i].get_x() + bars_dcr[i].get_width() / 2, bars_dcr[i].get_y() + bars_dcr[i].get_height()),
                     xytext=(0, 3),  # 3 points vertical offset
                     textcoords="offset points",
...
plt.savefig('img/dug_5_dasharo_releases_kpis.png', dpi=300)
