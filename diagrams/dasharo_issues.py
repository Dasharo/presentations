...
def count_issues(repo, state, date):
    ...
    command = f"gh issue list --repo {repo} --state {state} --search 'created:<={date}' -L 10000 --json number --jq '. | length'"
...
bars_open = plt.bar(
    dates, data["Open"], bottom=data["Closed"], color="#272727", label="Open issues"
)
...
plt.savefig("img/dug_11/dasharo_issues.png")
